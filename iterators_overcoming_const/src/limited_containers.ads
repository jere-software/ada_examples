...
package Limited_Containers is

   -- Main container type
   type Container(Count : Natural) is tagged limited private
...
private
   ...
   -- This is the "Rosen" type used for self referencing the container
   type Rosen(Self : not null access Container) is limited null record;
   ...
   with record
      ...
      -- Self reference implemented via the Rosen Technique.
      -- This is only available for limited records
      Proxy    : Rosen(Container'Access);  
